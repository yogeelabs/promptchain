...
for stage in $STAGES; do
  ...
  if [[ ! -d "$STAGE_DIR" ]]; then
    echo "Example run failed: stage directory missing for $stage" >&2
    exit 1
  fi

  if [[ ! -f "$RAW_FILE" ]]; then
    echo "Example run failed: raw output missing for $stage" >&2
    exit 1
  fi

  if [[ ! -f "$OUT_FILE" ]]; then
    echo "Example run failed: output.md missing for $stage" >&2
    exit 1
  fi
done

echo "Example run passed: $PIPELINE"
