...
if [[ ! -f "$BATCH_STATE" ]]; then
  echo "Smoke check failed: batch state missing for $MAP_STAGE" >&2
  exit 1
fi
...
if meta.get("execution_mode") != "batch":
    raise SystemExit("Smoke check failed: execution_mode is not batch")

state = json.loads(Path("$BATCH_STATE").read_text())
if not state.get("batch_id"):
    raise SystemExit("Smoke check failed: batch_id missing")
