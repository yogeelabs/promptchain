...
if [[ ! -f "$META_FILE" ]]; then
  echo "Smoke check failed: meta file missing for disabled stage" >&2
  exit 1
fi
...
if meta.get("status") != "skipped":
    raise SystemExit("Smoke check failed: disabled stage not marked skipped")
if meta.get("skip_reason") != "disabled_in_yaml":
    raise SystemExit("Smoke check failed: skip_reason not set to disabled_in_yaml")
PY

if ! rg -q "Stage ${DISABLED_STAGE} SKIPPED \\(disabled in pipeline yaml\\)" "$RUN_DIR/run.log"; then
  echo "Smoke check failed: skip log line missing" >&2
  exit 1
fi

if [[ ! -f "$RUN_DIR/stages/$ENABLED_STAGE/output.md" ]]; then
  echo "Smoke check failed: enabled stage output missing" >&2
  exit 1
fi
